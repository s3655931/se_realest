...
public class Main {
	...
	//TODO: fix bug here: trackers aren't part of persistence
	...
	static void newRentalProperty() {
		...
		System.out.printf(
				...
				address, suburb, customers.get(ownerId).getName(), price, leaseLength);
	}
	...
	static void editSaleProperty(String propertyId, String customerId) {
		...
		System.out.printf("Please enter new details for %s\n\n" + "Address (eg. \"123 Fake St\"):",
				saleProperties.get(propertyId).toString());
		address = sc.nextLine();
		System.out.print("Suburb:");
		suburb = sc.nextLine();
	...
	static String addRentalProperty(String id, String address, String suburb, String ownerId, double price,
			int leaseLength) {
		saleProperties.put(id, new SaleProperty(id, address, suburb, ownerId, price));
		return String.format(
	...
	static boolean applyForRental(String customerId) {
		...
		System.out.printf("Applying for %s\nWhat weekly rent would you like to offer? \n"
				...
				+ "Your weekly rent offer (in dollars and cents eg. 500.00):",
				rentalProperties.get(appliedForProperty).toString());
		while (!sc.hasNextDouble()) {
			System.out.println("Please input a valid price in the format \"dollar\".\"cents\"");
			sc.next();
		}
		offerPrice = sc.nextDouble();
		System.out.printf("\nSuccessfully offered $%,.2f weekly for %s.", offerPrice,
				rentalProperties.get(appliedForProperty).addressString());
	...
	static boolean applyForSale(String customerId) {
		Scanner sc = new Scanner(System.in);
		String appliedForProperty = null;
		while (true) {
			System.out.println("Please enter ID of Sale Property you'd like to apply for:");
			appliedForProperty = sc.next();
			if (!appliedForProperty.matches("P[0-9]{3}")) {
				System.out.println("Please enter a valid property ID");
			} else if (!saleProperties.containsKey(appliedForProperty)) {
				System.out.printf("Sale Property ID: %s does not exist\n", appliedForProperty);
			} else {
				break;
			}
		}
		double offerPrice = 0;
		System.out.printf("Applying for %s\nWhat dollar amount would you like to offer? (eg. 500000):",
				saleProperties.get(appliedForProperty).toString());
		while (!sc.hasNextDouble()) {
			System.out.println("Please input a valid price in the format \"dollar\".\"cents\"");
			sc.next();
		}
		offerPrice = sc.nextDouble();
		System.out.printf("\nSuccessfully offered $%,.2f for %s.", offerPrice,
				saleProperties.get(appliedForProperty).addressString());
		saleProperties.get(appliedForProperty).newOffer(customerId, offerPrice);
		customers.get(customerId).makeOffer(appliedForProperty, customerId);
		return true;
	}
	...
	static String getAllSales() {
		...
		saleProperties.forEach((key, value) -> result.append(String.format("ID: %s | %s\n", key, value.toString())));
		return result.toString();
	}

	static String getAllRentals() {
		StringBuilder result = new StringBuilder();
		rentalProperties.forEach((key, value) -> result.append(String.format("ID: %s | %s\n", key, value.toString())));
		return result.toString();
	}
	...
	static String findSalesBySuburb(String suburb) {
		StringBuilder result = new StringBuilder();
		result.append(String.format("Displaying all Sale Properties in %s.\n", suburb));
		for (SaleProperty property : saleProperties.values()) {
			if (property.compareSuburb(suburb)) {
				result.append(property.getId() + " | " + property.toString() + "\n");
			}
		}
	...
	static String getRentalOffersBy(String customerId) {
		StringBuilder sb = new StringBuilder();
		for (String offer : customers.get(customerId).getOffers()) {
			String property = offer.substring(0, 4);
			/*
			 * since offerID is P100C100 (propertyID + customerID), you can substring the
			 * offerId to get the property. iterating through the customer's offers array
			 * gives you the propertyid and offerid of each of their offers. using these,
			 * you can "get" the offer objects (from the main collection) for each
			 * discovered property and append their toString.
			 */
	...
	static String getSaleOffersBy(String customerId) {
		StringBuilder sb = new StringBuilder();
		for (String offer : customers.get(customerId).getOffers()) {
			String property = offer.substring(0, 4);
			/*
			 ...
			 * discovered property and append their toString.
			 */
			sb.append(String.format("%s(for %s.)\n", saleProperties.get(property).getOffers().get(offer).toString(),
					saleProperties.get(property).addressString()));
		}
		return sb.toString();
	}
	...
	static String getAllCustomers() {
		StringBuilder result = new StringBuilder();
		customers.forEach((key, value) -> result.append(String.format("ID: %s | %s\n", key, value.toString())));
		return result.toString();
	}

	static String getAllEmployees() {
		StringBuilder result = new StringBuilder();
		employees.forEach((key, value) -> result.append(String.format("ID: %s | %s\n", key, value.toString())));
		return result.toString();
	}
	...
	public static void logout() throws FileNotFoundException, IOException {
		// wipes the instances of Menu objects, removing any customer or employee data
		// from the cache
	...
	public static void inputData(HashMap<String, RentalProperty> rentalProperties,
			HashMap<String, SaleProperty> saleProperties, HashMap<String, Customer> customers,
		...
		while (inputCustomer.hasNextLine()) {
			...
			// }
			customers.put(textCustomer[0],
					new Customer(textCustomer[0], textCustomer[1], textCustomer[2], Integer.parseInt(textCustomer[3])));
		}
		inputCustomer.close();
		...
		while (inputEmployee.hasNextLine()) {
			...
			if (textEmployee.length == 5) {
				employees.put(textEmployee[0], new FullTimeEmployee(textEmployee[0], textEmployee[1], textEmployee[2],
						Double.parseDouble(textEmployee[3])));
				employees.get(textEmployee[0]).setEmployeeType(Integer.parseInt(textEmployee[4]));
			} else if (textEmployee.length == 4) {
				employees.put(textEmployee[0], new PartTimeEmployee(textEmployee[0], textEmployee[1], textEmployee[2],
						Double.parseDouble(textEmployee[3])));
			} else {
		...
		while (inputSale.hasNextLine()) {
			...
			// }
			saleProperties.put(textSale[0], new SaleProperty(textSale[0], textSale[1], textSale[2], textSale[3],
					Double.parseDouble(textSale[4])));
		...
		while (inputRent.hasNextLine()) {
			...
			// }
			rentalProperties.put(textRent[0], new RentalProperty(textRent[0], textRent[1], textRent[2], textRent[3],
					Double.parseDouble(textRent[4]), Integer.parseInt(textRent[5])));
