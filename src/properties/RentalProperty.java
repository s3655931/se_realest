...
public class RentalProperty extends Property {
	...
	private HashMap<String, Offer> rentalOffers = new HashMap<String, Offer>(); // collction of rental offers
																				// (applications)
	private HashMap<String, Inspection> rentalInspections = new HashMap<String, Inspection>(); // collection of
																								// inspections
	...
	// a property won't have a tenant upon creation, so call this when someone
	// successfully rents it
	...
	public String printOffers() {
		...
		this.rentalOffers.forEach((key, value) -> result.append(value.toString() + "\n"));
	...
	public String toString() {
		return String.format("%s %s, owned by %s, $%,.2f weekly, %s%s", this.address, this.suburb,
				Main.getCustomerName(this.ownerId), this.rentalPrice,
				this.onMarket ? "Available for rent. " : "Unavailable for rent. ",
				this.onMarket ? this.rentalOffers.size() + " offers." : "");
	}

	public String fileOutString() {
		String text = this.propertyId + "," + this.address + "," + this.suburb + "," + this.ownerId + ","
				+ this.rentalPrice + "," + this.leaseLength + "," + this.tenantId + "," + this.employeeId;
		return text;
	}
}
